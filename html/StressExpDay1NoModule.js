...
function PracticeBlockLoopLoopBegin(thisScheduler) {
  ...
  trialIterator = PracticeBlockLoop[Symbol.iterator]();
  while(true) {
    let result = trialIterator.next();
    if (result.done);
      break;
    let thisPracticeBlockLoop = result.value;
...
function Block1LoopLoopBegin(thisScheduler) {
  ...
  Block1Loop = new TrialHandler({
    ...
    nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: TrialHandler.importConditions(psychoJS.serverManager, 'StressWordLists.xlsx', '8:20'),
  ...
  // Schedule all the trials in the trialList:
  trialIterator = Block1Loop[Symbol.iterator]();
  while(true) {
    let result = trialIterator.next();
    if (result.done);
      break;
    let thisBlock1Loop = result.value;
...
function Block2LoopLoopBegin(thisScheduler) {
  ...
  Block2Loop = new TrialHandler({
    ...
    nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: TrialHandler.importConditions(psychoJS.serverManager, 'StressWordLists.xlsx', '93:103'),
  ...
  // Schedule all the trials in the trialList:
  trialIterator = Block2Loop[Symbol.iterator]();
  while(true) {
    let result = trialIterator.next();
    if (result.done);
      break;
    let thisBlock2Loop = result.value;
...
function Block3LoopLoopBegin(thisScheduler) {
  ...
  Block3Loop = new TrialHandler({
    ...
    nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: TrialHandler.importConditions(psychoJS.serverManager, 'StressWordLists.xlsx', '176:184'),
  ...
  // Schedule all the trials in the trialList:
  trialIterator = Block3Loop[Symbol.iterator]();
  while(true) {
    let result = trialIterator.next();
    if (result.done);
      break;
    let thisBlock3Loop = result.value;
...
function BaselineLoopLoopBegin(thisScheduler) {
  ...
  // Schedule all the trials in the trialList:
  trialIterator = BaselineLoop[Symbol.iterator]();
  while(true) {
    let result = trialIterator.next();
    if (result.done);
      break;
    let thisBaselineLoop = result.value;
...
function PracticeBlockRoutineBegin() {
  ...
  PracticeBlockComponents.forEach( function(thisComponent) {
    if ('status' in thisComponent)
      thisComponent.status = PsychoJS.Status.NOT_STARTED;
     });
...
function BlockIntroRoutineBegin() {
  ...
  BlockIntroComponents.forEach( function(thisComponent) {
    if ('status' in thisComponent)
      thisComponent.status = PsychoJS.Status.NOT_STARTED;
     });
...
function PreBaselineBreakRoutineBegin() {
  ...
  PreBaselineBreakComponents.forEach( function(thisComponent) {
    if ('status' in thisComponent)
      thisComponent.status = PsychoJS.Status.NOT_STARTED;
     });
...
function BaselineTestRoutineBegin() {
  ...
  BaselineTestComponents.forEach( function(thisComponent) {
    if ('status' in thisComponent)
      thisComponent.status = PsychoJS.Status.NOT_STARTED;
     });
